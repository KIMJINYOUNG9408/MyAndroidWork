...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 방법 3
        BackThread3 thread3 = new BackThread3(handler3); // 포인트 // 메인스레드의 Handler 객체를 외부 클래스에 넘겨줌.
        thread3.setDaemon(true);
        thread3.start();

        // 방법 4
        BackThread4 thread4 = new BackThread4(handler4); // 포인트 // 메인스레드의 Handler 객체를 외부 클래스에 넘겨줌.
        ...
        public void run() {
           while(true){
             // 방법1  ) 메인에서 생성된 Handler 객체의 sendEmptyMessage 를 통해 Message 전달
               backValue1++;
               handler1.sendEmptyMessage(1); // 방법 1 핵심.
...
// Message 를 수신합니다.
   Handler handler1 = new Handler(){
        ...
        public void handleMessage(@NonNull Message msg) {
            if(msg.what == 1){ // Message id가 1이면 what (id)
...
class BackThread3 extends Thread{
    ...
    public void run() {
        while(true){
            ...
            handler.sendMessage(msg); // 받은 handler. 생성할때.
...
class BackThread4 extends  Thread{
    ...
    public void run() {
        while(true){
            ...
            handler.sendMessage(msg); // 메인스레드에 메세지보내기
