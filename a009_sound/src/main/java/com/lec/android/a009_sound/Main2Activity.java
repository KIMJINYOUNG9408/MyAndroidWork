...
public class Main2Activity extends AppCompatActivity {
    ...
    int pos; // 재생 위치
    boolean isTrackig = false;

    class MyThread extends Thread {
        @Override
        public void run() {
            // SeekBar 가 음악 재생시 ,움직이게 하기
            while(mp.isPlaying()){ // 햔재 재생 중이라면
                pos = mp.getCurrentPosition(); // 현재 재생중인 위치 ms ( int )
                if(!isTrackig) sb.setProgress(pos); // seekBar 이동 -->  onProgressChanged() 호출함.
            }
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void onClick(View v) {
                ...
                mp.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                    ...
                    public void onCompletion(MediaPlayer mp) {
                        Log.d("mayapp","연주종료" + mp.getCurrentPosition() + "/" + mp.getDuration());
                        btnPlay.setVisibility(View.VISIBLE);
                        btnPause.setVisibility(View.INVISIBLE);
                        btnResume.setVisibility(View.INVISIBLE);
                        btnStop.setVisibility(View.INVISIBLE);
        ...
        btnResume.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mp.seekTo(pos); // 일시 정지시 위치로 이동 pos에 일시정지에 저장해두었음 위치.
                mp.start(); // 재생 시작
