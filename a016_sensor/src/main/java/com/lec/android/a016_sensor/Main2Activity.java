...
//        Sensor s = sm.getDefaultSensor(Sensor.TYPE_ORIENTATION); // 옛날방식

        accelerometer = sm.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
        magnetometer = sm.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD);
    }

    // 화면이 동작하기 직전에 센서 자원을 획득한다.
    @Override
    protected void onResume() {
        super.onResume();
        // 센서 값이 변경되었을때마다 콜백 받기 위한 리스너 등록. SensorEventListner 객체
