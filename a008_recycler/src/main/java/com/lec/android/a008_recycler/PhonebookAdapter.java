...
public class PhonebookAdapter extends RecyclerView.Adapter<PhonebookAdapter.ViewHolder> {
    ...
    // Adapter 생성자
    public PhonebookAdapter() { // 기본 생성자
        this.adapter = this;
    }
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
       ...
       // 주어진 ViewGroup 으로부터 LayoutInflater 추출
        ...
        //준비된레이아웃(XML)로 부터 View를 만들어 ViewGroup 에 붙이고 그렇게 만들어진 View를 리턴한다
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Phonebook item = items.get(position); // arrayList 에서 가져오기
        holder.setItem(item);
    }
    ...
    static class ViewHolder extends RecyclerView.ViewHolder{
        ...
        // ViewHolder에 담긴 각각의 View 들을 담을 변수
        ImageView ivPhoto;
        ...
        // 생성자 필수
        public ViewHolder(@NonNull View itemView) { // item 레이아웃의 View 객체가 전달된다.
            super(itemView);
            // 뷰 객체 가져오기
            ...
            btnDelItem.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    adapter.notifyDataSetChanged(); // 이거만 해주면 해결 ~!
            ...
            itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    int positon = getAdapterPosition(); // 이 리스너가 장착된 item의 리스트상의 position 값.
                    //Toast.makeText(v.getContext(),"position: " + positon, Toast.LENGTH_SHORT).show();

                    // 아이템을 클릭하면 해당 세부 정보 엑티비티로 넘겨주기
                    Intent intent = new Intent(v.getContext(),PhonebookDetail.class); // 엑티비티가 아니라 getApplication 안뜬다 .

                    intent.putExtra("pb",adapter.getItem(positon));
