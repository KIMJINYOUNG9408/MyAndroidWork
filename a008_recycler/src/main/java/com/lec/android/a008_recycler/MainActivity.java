...
//        RecyclerView.LayoutManager layoutManager = new GridLayoutManager(this,2); //   (2 개 씩 보 여 줌 )
    ...
    protected void insertData(View v) {
        int idx = D.next(); // 다음데이터

        // 리스트 맨 앞에 추가.
        adapter.addItem(0, new Phonebook(D.FACEID[idx],D.NAME[idx],D.PHONE[idx],D.EMAIL[idx])); // 0 번째에 추가.
